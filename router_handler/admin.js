...
exports.getUserList = (req, res)=>{
  const { pageNum, pageSize, search, role, adminId } = req.body
  ...
  try {
    ...
    actionLoger.log(ip, '查询用户列表', 1, 3, adminId)
  } catch {
  }
...
exports.delUser = (req, res)=>{
  ...
  db.query(sqlDel, userId, (err, result)=>{
    ...
    try {
      const ip = req.headers['x-forwarded-for'] || req.ip || 'unknown';
      actionLoger.log(ip, `删除账号${userId}`, 4, 3, adminId)
    } catch {
    }
  ...
  if(userInfo.id) {
    ...
    db.query(sqlcheck, userInfo.id, (err, checkResult)=>{ 
      ...
      if(checkResult[0].pwd===userInfo.pwd) {
        ...
        db.query(sqlEdit1, [ userInfo.userName, userInfo.balance, userInfo.id], (err, result)=>{
          ...
          try {
            const ip = req.headers['x-forwarded-for'] || req.ip || 'unknown';
            actionLoger.log(ip, `修改账号${userInfo.id}的信息`, 3, 3, adminId)
          } catch {}
          res.cc('账号信息已更新',200)
        })
      } else {
    ...
    db.query(sqlcheck, userInfo.account, (err, result)=>{  
      ...
      db.query(sqlinsert, {...userInfo}, (err,result)=>{
        ...
        try {
          const ip = req.headers['x-forwarded-for'] || req.ip || 'unknown';
          actionLoger.log(ip, `添加账号${userInfo.id}`, 2, 3, adminId)
        } catch {}
        res.cc('账号已添加', 200)
      })
    })
